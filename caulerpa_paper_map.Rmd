...
caulerpa_leaflet.plot <- caulerpa_leaflet.plot %>%
  onRender(sprintf("
    function(el, x) {
      ...
      // --- Helper: show the correct legend for a given base group name ---
      function updateLegend(groupName) {
        try { map.removeControl(reachLegend); } catch(e) {}
        try { map.removeControl(downLegend); } catch(e) {}

        var reachGroups = ['AIS Reachability', 'Survey Reachability'];
        var downGroups  = ['AIS Downstream',   'Survey Downstream'];

        if (reachGroups.indexOf(groupName) !== -1) {
          reachLegend.addTo(map);
        } else if (downGroups.indexOf(groupName) !== -1) {
          downLegend.addTo(map);
        }
      }

      // --- Initial legend: detect the currently visible base layer ---
      // iterate layers and pick the one that is both on the map and has a 'group' name
      ...
      // --- Update when base layer changes ---
      map.on('baselayerchange', function(e) {
        // e.name is the label used in addLayersControl(baseGroups = ...)
        updateLegend(e.name);
      });
    }
  ",
